...
def extract_json_from_markdown(raw_response):
    ...
    try:
        ...
        st.warning("No valid JSON content found in response")
        return {}
        
    except Exception as e:
        st.error(f"Error extracting JSON from markdown: {e}")
        st.write("Raw content:", raw_response)
        return {}
...
def poll_execution_status(execution_arn, max_attempts=30, delay=2):
    ...
    for _ in range(max_attempts):
        try:
            ...
            st.write("Debug - Step Functions Status:", status)
            
            if status == 'SUCCEEDED':
                output = response.get('output', '{}')
                st.write("Debug - Raw Step Functions Output:", output)
                ...
                try:
                    ...
                    return {
                        'status': 'ERROR',
                        'error': f"Invalid JSON output: {output}"
                    }
        ...
        with st.spinner('Starting analysis...'):
            ...
            if response.status_code == 200:
                ...
                if execution_arn:
                    ...
                    st.write("Debug - Final Status:", status)
...
def display_visual_analysis(analysis_data):
    ...
    if not analysis_data or not isinstance(analysis_data, dict):
        ...
        st.write("Debug - Visual Analysis Data:", analysis_data)
        return
...
def display_activity_pattern(pattern_data):
    ...
    if not pattern_data or not isinstance(pattern_data, dict):
        ...
        st.write("Debug - Activity Pattern Data:", pattern_data)
        return
...
def display_productivity_assessment(assessment_data):
    ...
    if not assessment_data or not isinstance(assessment_data, dict):
        ...
        st.write("Debug - Productivity Assessment Data:", assessment_data)
        return
        ...
        if compressed_file:
            ...
            if st.button('🔍 Analyze Productivity'):
                ...
                if image_base64:
                    ...
                    st.write("Debug - Base64 Size:", base64_size)
